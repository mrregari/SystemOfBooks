...
public class LibrarySystem
{
    ...
    {
        FileInfo book = new FileInfo(@"..\Books\books.txt");
        List<int> givedes = new List<int>();
        int maxValue;
        foreach(var book_ in books){
            int value = book_.Gived;
            givedes.Add(value);
        ...
        int maxId = 0;
        string maxTitle = " ";
        using(StreamReader reader = new StreamReader(@"..\Books\books.txt", true))
        {
            string line;
            int sum = 0;
            int id = 0;
            ...
            if (maxItem != null)
            {
                Console.WriteLine($"\nЭлемент с максимальным значением: {maxItem}");
            }
            else
            {
    ...
    public void LibMenu()
{
        ...
        switch (choice)
        {
            ...
            case "3":
                ...
                if (bookToUpdate != null)
                {
                    ...
                    AddLoanRecord(new LoanRecord(readerId, bookId, false, employeeId));
    ...
    public void AddTextFiles(){
        ...
        var readerlines = loanRecords.Select(l => $"{l.ReaderId}#{l.BookId}#{l.IsReturn}#{l.Date}");
